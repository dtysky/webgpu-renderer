...
fn genWorldRay(uv: vec2<f32>, gBInfo: GBufferInfo) -> Ray {
...
fn getGBInfo(uv: vec2<f32>) -> GBufferInfo {
  var info: GBufferInfo;
...
fn main(
  ...
  let gBInfo: GBufferInfo = getGBInfo(baseUV);
  ...
  let worldRay: Ray = genWorldRay(baseUV, gBInfo);
